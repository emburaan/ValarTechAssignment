package com.valartech.global.utils
...
import com.valartech.global.listener.DataAdapterListener
...
fun setImageUrl(imageView: ImageView?, imageUrl: String?, placeHolder: Drawable?, picasso: Picasso?) {
    if (TextUtils.isEmpty(imageUrl)) {
        ...
        when (imageView?.scaleType) {
            ImageView.ScaleType.CENTER_CROP -> placeHolder?.let {
                picasso?.load(imageUrl)?.fit()?.centerCrop()?.placeholder(
                    it
                )?.into(imageView)
            }
            ImageView.ScaleType.CENTER_INSIDE -> placeHolder?.let {
                picasso?.load(imageUrl)?.fit()?.centerInside()?.placeholder(
                    it
                )?.into(imageView)
            }
